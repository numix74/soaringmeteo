...
object LocationForecasts {
  ...
  /**
   * Create LocationForecasts from a sequence of UnifiedModelData.
   */
  def apply(forecasts: Seq[UnifiedModelData]): LocationForecasts = {
    val detailedForecasts: Seq[DetailedForecast] =
      forecasts.map { unifiedData =>
        DetailedForecast(
          unifiedData.time,
          unifiedData.xcFlyingPotential,
          unifiedData.boundaryLayerDepth,
          unifiedData.boundaryLayerWind,
          unifiedData.thermalVelocity,
          unifiedData.totalCloudCover,
          unifiedData.convectiveCloudCover,
          unifiedData.convectiveClouds,
          unifiedData.airDataByAltitude,
          unifiedData.mslet,
          unifiedData.snowDepth,
          unifiedData.surfaceTemperature,
          unifiedData.surfaceDewPoint,
          unifiedData.surfaceWind,
          unifiedData.totalRain,
          unifiedData.convectiveRain,
          unifiedData.latentHeatNetFlux,
          unifiedData.sensibleHeatNetFlux,
          unifiedData.cape.getOrElse(Grays(0)),  // Default to 0 if not available
          unifiedData.cin.getOrElse(Grays(0)),  // Default to 0 if not available
          unifiedData.downwardShortWaveRadiationFlux,
          unifiedData.isothermZero,
          unifiedData.winds  // Winds object already constructed in UnifiedModelData
        )
      }
    LocationForecasts(
