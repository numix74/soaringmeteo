 ...
 * Adapted from backend/common for use with unified pipeline.
 *
 * @param dataPath         Path prefix for generated file names (e.g., "7/gfs/20251229_06")
...
object ForecastMetadata {
   ...
   *
   * @param targetDir Model target directory (e.g., /output/7/gfs)
  ...
  def overwriteLatestForecastMetadata(
    ...
    val maybePreviousForecasts =
      for {
        ...
        str <- Try(os.read(latestForecastPath)).toOption
