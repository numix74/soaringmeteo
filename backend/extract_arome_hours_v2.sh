...
echo "=== Extraction AROME par heure individuelle (v2) ==="
...
extract_package() {
    ...
    while [ $hour -le $hour_end ]; do
        ...
        # La différence entre hour_start et hour donne l'offset dans le fichier
        local offset=$((hour - hour_start))
        
        if [ $offset -eq 0 ]; then
            # Pour la première heure (analyse)
            wgrib2 "$input_file" -match ":anl:" -grib "$output_file" 2>/dev/null
        else
            # Pour les prévisions
            wgrib2 "$input_file" -match ":${offset} hour fcst:" -grib "$output_file" 2>/dev/null
        fi
...
echo "SP1:"
ls -lh "$TARGET_DIR/sp1/" | grep -v "^total" | awk '{print $5, $9}' | head -15
echo ""
echo "SP2:"
ls -lh "$TARGET_DIR/sp2/" | grep -v "^total" | awk '{print $5, $9}' | head -15
echo ""
echo "SP3:"
ls -lh "$TARGET_DIR/sp3/" | grep -v "^total" | awk '{print $5, $9}' | head -15
echo ""
echo "Nombre de fichiers non-vides par package:"
echo "SP1: $(find "$TARGET_DIR/sp1/" -type f -size +0 | wc -l) / $(ls "$TARGET_DIR/sp1/" | wc -l)"
echo "SP2: $(find "$TARGET_DIR/sp2/" -type f -size +0 | wc -l) / $(ls "$TARGET_DIR/sp2/" | wc -l)"
echo "SP3: $(find "$TARGET_DIR/sp3/" -type f -size +0 | wc -l) / $(ls "$TARGET_DIR/sp3/" | wc -l)"
