# MeteoData Refactoring - Technical Documentation
...
> **Note (Novembre 2025)** : Le type `Forecast` est maintenant aliasé comme `GfsData` dans le module GFS
> pour clarifier qu'il s'agit de données spécifiques au modèle GFS. Voir `backend/gfs/package.scala`.
> Un renommage complet est prévu pour une future version majeure.
